...
def entropy_post_rewards(entropy, amplify=1.0):
    """
    计算奖励：entropy越高reward越小，reward有正负，正值奖励，负值惩罚。
    :param entropy: torch.Tensor, shape (...), 熵值
    :param amplify: float, 放大系数
    :return: torch.Tensor, shape (...), 奖励
    """
    mean = torch.mean(entropy, dim=-1, keepdim=True)
    if amplify > 0:
        std = torch.std(entropy, dim=-1, keepdim=True) + 1e-6
    else:
        std = 1.0
    # 熵越高，奖励越小，反向
    rewards = -(entropy - mean) / std 
...
def forward(
    ...
    for decoder_layer in self.layers:
        ...
        # print('hidden_states:{}'.format(hidden_states.shape)) # [576, 4096]
        ...
        if vision_retracing_sign == True:
            with torch.no_grad():
                surgery_entropy = -probabilities[:10] * torch.log(probabilities[:10])/np.log(10)
                surgery_entropy = torch.sum(surgery_entropy, dim=1)
                # print('surgery_entropy:{}'.format(surgery_entropy)) 
                rewards = entropy_post_rewards(surgery_entropy, amplify=0)
        ...
        # print('pre vr hidden_states:{}'.format(hidden_states)) # [576, 4096]

        if entropy > entropy_threshold and visual_retracing_event == False and layer > starting_layer and layer < ending_layer:
            layer_visual_token = visual_token
            # print('layer_visual_token:{}'.format(layer_visual_token)) # 
            # print('self.surgery_down_proj:{}'.format(self.surgery_down_proj)) # 
            sub_layer_visual_token = self.surgery_down_proj(layer_visual_token)
            sub_layer_visual_token = self.surgery_relu(sub_layer_visual_token)
            sub_layer_visual_token = self.surgery_up_proj(sub_layer_visual_token)

            out_layer_visual_token = layer_visual_token + sub_layer_visual_token
            # out_layer_visual_token = self.norm(sub_layer_visual_token)
            # print('out_layer_visual_token:{}'.format(out_layer_visual_token)) # [576, 512]
            ...
            # self.layers[layer+1].mlp.adpt_w1 += (torch.mean(torch.abs(self.layers[layer+1].mlp.up_proj.weight)) / (torch.mean(torch.abs(out_layer_visual_token)))) * out_layer_visual_token
            # self.layers[layer+1].mlp.adpt_w2 += (torch.mean(torch.abs(self.layers[layer+1].mlp.down_proj.weight)) / (torch.mean(torch.abs(out_layer_visual_token)))) * out_layer_visual_token.T

            adpt_w1_value = (torch.mean(torch.abs(self.layers[layer+1].mlp.up_proj.weight)) / torch.mean(torch.abs(out_layer_visual_token))) * out_layer_visual_token
            adpt_w2_value = (torch.mean(torch.abs(self.layers[layer+1].mlp.down_proj.weight)) / torch.mean(torch.abs(out_layer_visual_token))) * out_layer_visual_token.T
            self.layers[layer+1].mlp.adpt_w1 = torch.nn.Parameter(adpt_w1_value.detach())
            self.layers[layer+1].mlp.adpt_w2 = torch.nn.Parameter(adpt_w2_value.detach())

            # print('layer:{}, hidden_states:{}'.format(layer, hidden_states)) # [576, 4096]
    ...
    # print('shape:{}, hidden_states:{}'.format(hidden_states.shape, hidden_states)) 
...
def apply_memvr_llama(
    ...
    self.model.surgery_down_proj = nn.Linear(4096, 512, bias=False).to(device).to(torch.float16)
    self.model.surgery_up_proj = nn.Linear(512, 4096, bias=False).to(device).to(torch.float16)
    torch.nn.init.kaiming_uniform_(self.model.surgery_down_proj.weight, a=math.sqrt(5))
    torch.nn.init.zeros_(self.model.surgery_up_proj.weight)
    self.model.surgery_relu = torch.nn.ReLU().to(device).to(torch.float16)

    self.model.trainable_params = []
    self.model.trainable_params.append(self.model.surgery_down_proj.weight)
    self.model.trainable_params.append(self.model.surgery_up_proj.weight)

    for layer in range(31):
        self.model.layers[layer].mlp.retracing_ratio = retracing_ratio

@torch.no_grad()
def reset_surgery(self):
    torch.nn.init.kaiming_uniform_(self.model.surgery_down_proj.weight, a=math.sqrt(5))
    torch.nn.init.zeros_(self.model.surgery_up_proj.weight)
